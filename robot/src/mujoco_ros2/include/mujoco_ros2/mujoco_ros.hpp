...
/**
 * @brief This class launches both a MuJoCo simulation, and ROS2 node for
 * communication.
 */
...
public:
  /**
   * @brief Contructor.
   * @param xmlLocation Where to find the XML file that defines the MuJoCo
   * model.
   * @param jointStateTopicName The name of the topic to which joint state data
   * is published.
   * @param jointControlTopicName The name of the topic to subscribe to for
   * control inputs.
   * @param controlMode POSITION, VELOCITY, or TORQUE
   * @param simulationFrequency The rate in Hz to update the simulation
   * @param visualisationFrequency The rate with which to refresh the 3D
   * rendering of the robot, environment
   */
...
private:
  std::vector<double>
      _torqueInput; ///< Used to store joint commands in torque mode
  ...
  rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr
      _jointStatePublisher; ///< As it says on the label

  rclcpp::Subscription<std_msgs::msg::Float64MultiArray>::SharedPtr
      _jointCommandSubscriber; ///< Subscriber for joint commands

  rclcpp::TimerBase::SharedPtr _simTimer,
      _visTimer; ///< Regulates the ROS2 node

  sensor_msgs::msg::JointState
      _jointStateMessage; ///< For publishing joint state data over ROS2
